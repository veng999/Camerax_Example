...
private val IMAGE_FORMAT_YUV_420_888 = ImageFormat.YUV_420_888
...
class YuvToRgbConverter(context: Context) {
    ...
    fun yuvToRgb(image: Image, output: Bitmap) {
        ...
        if (!::yuvBuffer.isInitialized) {
            ...
            yuvBuffer = ByteBuffer.allocateDirect(
                pixelCount * ImageFormat.getBitsPerPixel(IMAGE_FORMAT_YUV_420_888) / 8
            )
        }
    ...
    private fun imageToByteBuffer(image: Image, outputBuffer: ByteBuffer) {
        //Check the format of input image
        assert(image.format == IMAGE_FORMAT_YUV_420_888)

        //Get pixels related with this image
        val imageCrop = image.cropRect
        //Get array of images's plane (A single color plane of image data.)
        val imagePlanes = image.planes
        //get distance between the start of two consecutive rows of pixels in the image
